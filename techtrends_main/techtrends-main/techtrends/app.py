...
@app.route("/about")
def about():
    logging.debug("About page retrieved!")
...
def create():
    if request.method == "POST":
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            logging.debug('Article %s has been created!',title)
...
def initialize_logger():
    ...
    # Create a handler to capture stdout and add it to the logger
    stdout_handler = logging.StreamHandler(sys.stdout)
    stdout_handler.setLevel(log_level)
    stdout_handler.setFormatter(logging.Formatter('%(levelname)s:%(name)s:%(asctime)s, %(message)s'))
    logging.getLogger().addHandler(stdout_handler)

    # Create a handler to capture stderr and add it to the logger
    stderr_handler = logging.StreamHandler(sys.stderr)
    stderr_handler.setLevel(logging.ERROR)  # Adjust the level as needed
    stderr_handler.setFormatter(logging.Formatter('%(levelname)s:%(name)s:%(asctime)s, %(message)s'))
