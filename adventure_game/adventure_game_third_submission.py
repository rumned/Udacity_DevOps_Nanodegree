...
def boss_fight(items):
    ...
    def skill_check():
        print_pause("\n")
        print("Your chance of hitting: " + str(attack))
        print("Your attack target: " + accuracy)
        print("Successful hits: " + str(hit_count))
    ...
    if "cat" in items:
        while hit_count < 4 and hp > 0:
            ...
            accuracy = "Heads"
            skill_check()
            if attack >= 3 and accuracy == "Heads":
                attack_success_text()
                hit_count += 1
            elif hp == 0:
                hp_zero()
