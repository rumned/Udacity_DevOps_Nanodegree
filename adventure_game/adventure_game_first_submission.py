...
def intro():
    ...
    print_pause("You wake up, still feeling disoriented." 
                "You look around and you find 2 magical creatures in front of you.")

def adventure():
    print_pause("With your chosen companion, you roam through the land,")
    print_pause("in search of quests to prove your worth.")
    print_pause("Suddenly, you feel warmth in the air. At first it was pleasant,")
    ...
    if "cat" in items:
        while hit_count < 4 and hp > 0:
            ...
            if attack >= 4 or accuracy == "Heads":
                ...
                print_pause("The dragon attacks back, but you gallantly parry the attack!")
                hit_count += 1
            elif hp == 0:
                print_pause("With all your might, you launched a blinding fast attack from downwards...")
                print_pause("But the dragon saw it, and blasted you with its fire...")
                print_pause("You have lost.")
                game_over = input("Do you want to play the game again?\n" 
                                          "Type yes if you want to play, anything else to exit.").lower()
                if "yes" in game_over:
                    play_game()
            else:
                print_pause("You tried to strike, but the dragon...")
                print_pause("Parried your attack and slammed you to the ground!")
                ...
                if hp == 0:
                    ...
                    if "yes" in game_over:
                        play_game()
        if hit_count == 4:
                print_pause("With all your might, you launched a blinding fast attack from upwards...")
                print_pause("and you sliced off the dragon's wings!")
                print_pause("As he writhed in pain, you took advantage and lunge straight at his head...")
